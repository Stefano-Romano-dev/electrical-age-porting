...
Lo scheduler legacy usa tre accumulatori temporali e sceglie il prossimo sottopasso confrontando `electricalTimeout` e `thermalTimeout`. A parità iniziale privilegia l'elettrico (`<=`), perciò il primo tick include un solve a tempo zero e uno al limite del tick; dai tick successivi il ritmo si assesta. Prima e dopo il loop esegue, nell'ordine, slow-pre, termico slow fisso a 0,05 s, slow, distruzioni differite e slow-post. Il `Simulator` moderno conserva questa sequenza senza dipendere dall'evento server; `ServerSimulationLifecycle` possiede un simulatore per identità di `MinecraftServer` e invoca `tick()` su `ServerTickEvent.Pre`.
