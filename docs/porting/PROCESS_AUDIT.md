...
- controparti pure implementate o sostituzioni architetturali equivalenti: 32;
- tipi rimandati al layer che possiede le loro dipendenze: 10;
...
| `Simulator` | `Simulator` + `ThermalSimulator` + adapter ambiente | verificato, integrazione NeoForge rimandata |
...
| `NodeVoltageState` | adapter persistenza stato elettrico | rimandato: unica responsabilità aggiuntiva è NBT |
...
Il prossimo audit deve partire dai 10 tipi rimandati seguendo le relative vertical slice, non copiandoli anticipatamente nel core. Prima di chiudere M1 restano inoltre da definire codec `CompoundTag` per gli snapshot già mappati e una prova dell'adapter tick su dedicated server.
