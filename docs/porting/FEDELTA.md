...
| Configurazione sorgente M3 | conserva `voltage`, nessun clamp arbitrario; valori non finiti rifiutati al confine server | come `GuiTextFieldEln`, commit su Invio/perdita focus e nessun invio durante la digitazione | primo menu a campo singolo localizzato, parsing/precisione legacy dipendenti dal locale; texture GUI legacy non applicabile perché l'originale usava il pannello generico | non applicabile | payload server-bound vincolato a menu/posizione/faccia/distanza; update BE di ritorno verificato | 174 test, codec payload + persistenza + probe widget→dedicated→client a `123,5 V` con locale `it_IT` | coperto per il primo slice M3 |
| Multimetro M3 | misure dal runtime server per sorgente, cavo LV e resistore; unità e precisione di `Utils.plotUIP`/`plotOhm` | clic puntuale, senza consumo dell'item | texture 16×16 canonica integrata; confronto affiancato dell'item ancora aperto | nessun feedback aggiuntivo inventato | item sincronizzato al client; clic client→server dopo correzione della precedenza SixNode ancora da verificare | test formatter, grafo e GameTest; build 177 test e 11/11 GameTest | parziale |
