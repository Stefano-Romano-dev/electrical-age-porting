...
| Sorgente elettrica | `192` (`3 << 6` + `0`) | `eln:electrical_source` | M2 verificata, configurazione M3 integrata | sorgente monopolo, chiave `voltage`, resistenza seriale, gruppo `main`, texture e montaggio sulle sei facce coperti; menu/payload server-authoritative e widget numerico locale verificati, LED resta nel catalogo M4 |
| Multimetro | `896` (`14 << 6` + `0`) | `eln:multimeter` | M3 parziale | item e texture legacy integrati; lettura server di cavo, sorgente e resistore e formattazione chat testate; clic multiplayer finale e confronto visivo affiancato aperti |
